...
} from "../../model/schema/useRegisterFormSchema";
...
const RegisterForm = () => {
  ...
  return (
    <HStack justify="center">
      <VStack className={cls.register} gap={12}>
        <VStack className={cls.card} max gap={20} align="center">
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            <VStack gap={16}>
              <VStack gap={16} max>
                <Input
                  {...register(RegisterFieldsNames.EMAIL)}
                  value={watch(RegisterFieldsNames.EMAIL)}
                  error={errors?.email?.message}
                  placeholder="Телефон, имя пользователя или эл.адрес  "
                />
                <Input
                  {...register(RegisterFieldsNames.FULLNAME)}
                  value={watch(RegisterFieldsNames.FULLNAME)}
                  error={errors?.fullname?.message}
                  placeholder="Имя и фамилия"
                />
                <Input
                  {...register(RegisterFieldsNames.USERNAME)}
                  value={watch(RegisterFieldsNames.USERNAME)}
                  error={errors?.username?.message}
                  placeholder="Имя пользователя"
                />
                <Input
                  {...register(RegisterFieldsNames.PASSWORD)}
                  value={watch(RegisterFieldsNames.PASSWORD)}
                  error={errors?.password?.message}
                  type="password"
                  placeholder="Пароль"
                />
                <Input
                  {...register(RegisterFieldsNames.CF_PASSWORD)}
                  value={watch(RegisterFieldsNames.CF_PASSWORD)}
              ...
              <Button max type="submit" variant="solid" disabled={!isValid}>
                Войти
              </Button>
            </VStack>
          </Form>

          <Text size={12} fw={500} as="span">
            Забыли пароль?
          </Text>
        </VStack>
